...
void BDT_Regression( TString myMethodList = "" )
{
 ...
 /* 
   dataloader->AddVariable( "Cppf_phi1", "Cppf_phi1", "units", 'S' );
   dataloader->AddVariable( "Cppf_phi2", "Cppf_phi2", "units", 'S' );
   dataloader->AddVariable( "Cppf_phi3", "Cppf_phi3", "units", 'S' );
   dataloader->AddVariable( "Cppf_phi4", "Cppf_phi4", "units", 'S' );
*/
   /*
   dataloader->AddVariable( "Cppf_theta1", "Cppf_theta1", "units", 'S' );
   dataloader->AddVariable( "Cppf_theta2", "Cppf_theta2", "units", 'S' );
...
*/
   ...
   dataloader->AddVariable( "Deltatheta34", "Deltatheta34", "units", 'S' );
*/
   //dataloader->AddVariable( "NCPPFDigis", "NCPPFDigis", "units", 'S'); 
   ...
   TTree *regTree = (TTree*)input->Get("demo/tree");
   ...
   if (Use["DNN_CPU"]) {
      ...
      TString training0("LearningRate=1e-5,Momentum=0.5,Repetitions=1,ConvergenceSteps=100,BatchSize=50,"
                        "TestRepetitions=7,WeightDecay=0.01,Regularization=NONE,DropConfig=0.5+0.5+0.5+0.5,"
                        "DropRepetitions=2");
    /*  TString training1("LearningRate=1e-5,Momentum=0.9,Repetitions=1,ConvergenceSteps=20,BatchSize=30,"
                        "TestRepetitions=7,WeightDecay=0.01,Regularization=L2,DropConfig=0.1+0.1+0.1,DropRepetitions="
                        "1");
      TString training2("LearningRate=1e-5,Momentum=0.3,Repetitions=1,ConvergenceSteps=20,BatchSize=40,"
                        "TestRepetitions=7,WeightDecay=0.01,Regularization=NONE");
      TString training3("LearningRate=1e-6,Momentum=0.1,Repetitions=1,ConvergenceSteps=20,BatchSize=100,"
...
*/
   ...
   if (Use["BDT"])
     factory->BookMethod( dataloader,  TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=2000:MinNodeSize=0.1%:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=20:PruneMethod=CostComplexity:PruneStrength=30:MaxDepth=4" );

   if (Use["BDTG"])
     factory->BookMethod( dataloader,  TMVA::Types::kBDT, "BDTG",
                           "!H:!V:NTrees=2000::BoostType=Grad:Shrinkage=0.1:UseBaggedBoost:BaggedSampleFraction=0.3:nCuts=20:MaxDepth=4" );
   // --------------------------------------------------------------------------------------------------
